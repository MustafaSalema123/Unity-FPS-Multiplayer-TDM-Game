...
using UnityEngine.UI;
    ...
    public class GunController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            //if (/*Input.GetButtonDown("Fire1")*/inputManager.fireInput && !firing && reloading == false && bulletsInMag > 0 && !cycling && !swapping)
            //{
            //    firing = true;
            ...
            //    }
            //}
            //else if (inputManager.fireInput == false || bulletsInMag == 0)
            //{
            //    firing = false;
            //    recoilSemi = false;
            //    recoilAuto = false;
            ...
            //if (inputManager.reloadInput && !reloading && !firing && bulletsInMag < bulletsPerMag && totalBullets > 0)
            //{
        ...
        public void ShootGunUpdate(bool fireInp , bool reloadInp , int playerTeam) 
        {
            if (/*Input.GetButtonDown("Fire1")*/fireInp && !firing && reloading == false && bulletsInMag > 0 && !cycling && !swapping)
            {
                firing = true;
                foreach (ParticleSystem ps in muzzleFlashes)
                {
                ...
                if (shootType == ShootTypes.FullAuto)
                {
                    ...
                    recoilAuto = true;
                    recoilSemi = false;
                    lastRoutine = StartCoroutine(shootBullet(playerTeam));
                }
                else if (shootType == ShootTypes.SemiAuto)
                {
                    spawnShell();
            ...
            else if (fireInp == false || bulletsInMag == 0)
            {
                firing = false;
                recoilSemi = false;
                recoilAuto = false;
            ...
            if (reloadInp && !reloading && !firing && bulletsInMag < bulletsPerMag && totalBullets > 0)
            {
                anim.SetBool("reload", true);
                reloading = true;
                gameObject.GetComponent<AudioSource>().PlayOneShot(reloadSound);
        ...
        public void ScopeWeaponLateUpdate(bool scopeInp) 
        {

            if (mainCam == null) return;
            
                if (/*Input.GetButtonDown("Fire2")*/  scopeInp  && aimFinished && !swapping)
            {
            ...
            else if (/*Input.GetButtonUp("Fire2") */ !scopeInp && aiming && !swapping)
            {
                aiming = false;
                aimTimeElapsed = 0;
        ...
        IEnumerator shootBullet(int pt)
        {
            while (true)
            {
    ...
    void reloadFinished()
        ...
        void spawnBullet(int playerTeam)
        {
            GameObject tempBullet;
            if (shootFromCamera)
            {
                //Spawn bullet from the camera shoot point position, not from the true tip of the gun
                tempBullet = Instantiate(Bullet, shootPointCamera.transform.position, shootPointCamera.transform.rotation) ;
            tempBullet.GetComponent<registerHit>().damage = Damage;
            tempBullet.GetComponent<registerHit>().playerTeam = playerTeam;
        }
            else
            {
                //Spawn bullet from the shoot point position, the true tip of the gun
                tempBullet = Instantiate(Bullet, shootPoint.transform.position, shootPoint.transform.rotation) ;
            tempBullet.GetComponent<registerHit>().damage = Damage;
            tempBullet.GetComponent<registerHit>().playerTeam = playerTeam;
        }
